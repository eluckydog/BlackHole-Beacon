"""
BlackHole Beacon — Hybrid Anomaly Detection v8.0

Uses BOTH IR colors AND other features (PM, variability, compactness).
WHY? Because pulsars/BH may NOT be anomalous in IR ONLY.
"""
...
# 1. Load data
# ==========
print("BlackHole Beacon — Hybrid Anomaly Detection v8.0")
print("="*80)
...
# ==========
# 2. Build feature matrix (HYBRID features)
# ==========
print("\n--- 2. Build Feature Matrix (HYBRID features) ---")

def extract_features(candidate):
    """Extract 9 features (IR colors + PM + variability + compactness)."""
    features = []
    ...
    # 5-9: Other features (PM, variability, compactness)
    pm_total = candidate.get("proper_motion_masyr", 0.0) or 0.0
    
    # Variability (from Phase 2 analysis)
    variability = candidate.get("variability", 0.0) or 0.0
    
    # Compactness (from Phase 3 scoring)
    compactness = candidate.get("scores", {}).get("compactness", 0.0) or 0.0
    
    # Score components (from Phase 3)
    score_color = candidate.get("scores", {}).get("color", 0.0) or 0.0
    score_pm = candidate.get("scores", {}).get("pm", 0.0) or 0.0
    
    features.extend([pm_total, variability, compactness, score_color, score_pm])
...
print(f"  Features: 9 (IR colors 1-4, PM 5, variability 6, compactness 7, score_color 8, score_pm 9)")

# ==========
# 3. Train Isolation Forest (HYBRID features)
# ==========
print("\n--- 3. Train Isolation Forest (HYBRID features) ---")
...
# Isolation Forest
print("  Training Isolation Forest (HYBRID features)...")
iso_forest = IsolationForest(
    n_estimators=200,
    max_samples='auto',
    contamination=0.05,  # Assume 5% are anomalies (reduce false positives)s/BH)
    random_state=42,
    n_jobs=-1
)
...
fig.suptitle("BlackHole Beacon — Hybrid Anomaly Detection v8.0", fontsize=16)
...
ax.set_title("Top 20 Anomalies (v8)")
ax.invert_yaxis()

# 4. Feature importance (PCA loadings)
ax = axes[1, 0]
feature_names = ["J-H", "H-K", "W1-W2", "W2-W3",
                 "PM", "variability", "compactness", "score_color", "score_pm"]
...
plt.tight_layout()
plot_file = os.path.join(PLOT_DIR, "classifier_results_v8.png")
...
import pickle
model_file = os.path.join(DATA_DIR, "classifier_model_v8.pkl")
...
# ==========
print("\n" + "="*80)
print("BlackHole Beacon — Hybrid Anomaly Detection v8.0 COMPLETE")
print("="*80)
...
print("\n" + "="*80)
print("DONE. Hybrid anomaly detection complete (IR + PM + variability + compactness).")
print("="*80)
