"""
BlackHole Beacon — Classifier Training v4.1 (Fixed extract_features)

Uses REAL negative samples (confusing sources) instead of synthetic ones.
This should reduce overfitting (previous AUC=1.0000 was due to BUGGY extract_features()).

FIXED:
- Positive samples: proper_motion_masyr is a SCALAR (total PM)
- Negative samples: pm_ra, pm_dec are separate
- IR colors: top-level keys (J, H, K, W1, W2) for positive samples
"""
...
# 1. Load data
# ==========
print("BlackHole Beacon — Classifier Training v4.1 (Fixed extract_features)")
print("="*80)
...
def extract_features(candidate, is_positive=True):
    """Extract 12 features from a candidate (positive or negative)."""
    features = []
    ...
    # 10-14: Phase 3 scores
    if is_positive:
        scores = candidate.get("scores", {})
        score_total = scores.get("total", 0.0) or 0.0
        score_color = scores.get("color", 0.0) or 0.0
        score_pm = scores.get("proper_motion", 0.0) or 0.0
        score_var = scores.get("variability", 0.0) or 0.0
        score_compact = scores.get("compactness", 0.0) or 0.0
    else:
        # Negative samples don't have Phase 3 scores → set to 0
        score_total = 0.0
        score_color = 0.0
        score_pm = 0.0
        score_var = 0.0
    ...
    features.extend([score_total, score_color, score_pm, score_var, score_compact])
...
feature_names = ["J-H", "H-K", "W1-W2", "W2-W3",
                 ...
                 "score_total", "score_color", "score_pm", "score_variability", "score_compactness"]
...
# Save ranking
ranking_file = os.path.join(DATA_DIR, "classifier_ranking_v4.json")
...
fig.suptitle("BlackHole Beacon — Classifier Evaluation v4.1 (Fixed)", fontsize=16)
...
ax.set_title("Top 20 Candidates (v4.1)")
ax.invert_yaxis()
...
plt.tight_layout()
plot_file = os.path.join(PLOT_DIR, "classifier_results_v4.png")
...
import pickle
model_file = os.path.join(DATA_DIR, "classifier_model_v4.pkl")
...
# ==========
print("\n" + "="*80)
print("BlackHole Beacon — Classifier Training v4.1 COMPLETE")
print("="*80)
