"""
BlackHole Beacon — Classifier Training v6.0 (IR colors ONLY)

Uses REAL negative samples (confusing sources) instead of synthetic ones.
REMOVED ALL PM features (pm_ra, pm_dec, pm_total, etc.).
Now uses ONLY IR colors (4 features):
1. J-H (2MASS J - H)
2. H-K (2MASS H - K)
3. W1-W2 (WISE W1 - W2)
4. W2-W3 (WISE W2 - W3)

This is the ULTIMATE test: can the classifier separate pulsars from confusers
using IR colors ONLY?
If AUC = 1.0000 → IR colors also perfectly separable (surprising!)
If AUC < 1.0000 → Finally! Not overfitting!
"""
...
# 1. Load data
# ==========
print("BlackHole Beacon — Classifier Training v6.0 (IR colors ONLY)")
print("="*80)
...
# ==========
# 2. Build feature matrix (IR colors ONLY!)
# ==========
print("\n--- 2. Build Feature Matrix (IR colors ONLY!) ---")

def extract_features(candidate, is_positive=True):
    """Extract ONLY 4 IR color features (NO PM, NO scores!)."""
    features = []
    ...
    # NO PM features (removed!)
    # NO Phase 3 scores (removed!)
...
print(f"  Features: 4 (IR colors ONLY! NO PM, NO scores)")
...
# Extract features for ALL candidates (IR colors ONLY)
...
# Save ranking
ranking_file = os.path.join(DATA_DIR, "classifier_ranking_v6.json")
...
fig.suptitle("BlackHole Beacon — Classifier Evaluation v6.0 (IR colors ONLY)", fontsize=16)
...
ax.set_title("Feature Importance (4 features)")
ax.invert_yaxis()
...
ax.set_title("Top 20 Candidates (v6)")
ax.invert_yaxis()
...
plt.tight_layout()
plot_file = os.path.join(PLOT_DIR, "classifier_results_v6.png")
...
import pickle
model_file = os.path.join(DATA_DIR, "classifier_model_v6.pkl")
...
# ==========
print("\n" + "="*80)
print("BlackHole Beacon — Classifier Training v6.0 COMPLETE")
print("="*80)
