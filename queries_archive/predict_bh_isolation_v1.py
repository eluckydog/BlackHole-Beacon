#!/usr/bin/env python3
"""
Predict v9.0 candidates using Isolation Forest trained on BlackCAT.
Rank candidates by "black-hole-ness" (anomaly score).
"""
...
def predict_bh_isolation(candidates, model_path="data/bh_isolation_forest_v1.pkl"):
    """Predict "black-hole-ness" for candidates."""
    ...
    X_imputed = imputer.transform(X)  # Use transform (not fit_transform) - imputer was already fit on training data
    
    # Predict anomaly scores (higher = more "black-hole-like")
    scores = clf.score_samples(X_imputed)
    probabilities = clf.predict(X_imputed)  # -1 = outlier (black hole), 1 = inlier (not black hole)
    
    # Convert to "black hole probability" (0 = not BH, 1 = BH)
    # Isolation Forest: -1 = outlier, 1 = inlier
    # We want: higher = more likely BH
    # So: p(BH) = 1 if prediction = -1, else 0
    # Actually, use anomaly score directly (more negative = more "black-hole-like")
    
    # Combine with candidate info
    results = []
    for i, cand in enumerate(valid_candidates):
        # Anomaly score: more negative = more "black-hole-like"
        # Convert to "BH probability" (0-1, higher = more likely BH)
        # Use sigmoid-like transformation: p(BH) = 1 / (1 + exp(score))
        # Since score is negative, p(BH) will be > 0.5 for "black-hole-like"
        score = scores[i]
        proba_bh = 1.0 / (1.0 + np.exp(score))
        
        result = {
            "anchor": cand.get("anchor", "Unknown"),
            "proba_bh": float(proba_bh),
...
def save_predictions(results, output_path="data/v9_candidates_bh_isolation_v1.json"):
    ...
    output = {
        "metadata": {
            "model": "bh_isolation_forest_v1.pkl",
            "training_samples": 20,
            "training_source": "BlackCAT (known black hole X-ray binaries)",
...
def print_top_20(results, title="Top 20 Candidates by p(BH) - Isolation Forest v1"):
...
def main():
    print("="*60)
    print("Black Hole Prediction - Isolation Forest v1")
    print("="*60)
    ...
    # Step 2: Predict "black-hole-ness"
    results = predict_bh_isolation(
        candidates,
        model_path="data/bh_isolation_forest_v1.pkl"
    )
    
    # Step 3: Save predictions
    output_path = save_predictions(
        results,
        output_path="data/v9_candidates_bh_isolation_v1.json"
    )
    ...
    print("SUMMARY")
    print("="*60)
    print(f"Model: bh_isolation_forest_v1.pkl")
    print(f"Training samples: 20 (BlackCAT sources)")
