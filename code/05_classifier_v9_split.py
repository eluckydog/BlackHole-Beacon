"""
BlackHole Beacon — Anomaly Detection v9.0 (remove PM features)

Remove PM features because all TOP 10 have PM=0.0 (no information).
Use ONLY IR colors + variability + compactness.
"""
...
# 1. Load data
# ==========
print("BlackHole Beacon — Anomaly Detection v9.0 (no PM)")
print("="*80)
...
# ==========
# 2. Build feature matrix (NO PM features)
# ==========
print("\n--- 2. Build Feature Matrix (NO PM) ---")

def extract_features(candidate):
    """Extract 7 features (IR colors + variability + compactness)."""
    features = []
    ...
    features.extend([variability, compactness, score_color])
...
print(f"  Features: 7 (IR colors 1-4, variability 5, compactness 6, score_color 7)")

# ==========
# 3. Train Isolation Forest (NO PM)
# ==========
print("\n--- 3. Train Isolation Forest (NO PM) ---")
...
# Isolation Forest
print("  Training Isolation Forest (NO PM)...")
iso_forest = IsolationForest(
    n_estimators=200,
    max_samples='auto',
    contamination=0.05,  # Assume 5% are anomalies (reduce false positives)
    random_state=42,
    n_jobs=-1
)
...
# Save ranking
ranking_file = os.path.join(DATA_DIR, "classifier_ranking_v9.json")
...
fig.suptitle("BlackHole Beacon — Anomaly Detection v9.0 (NO PM)", fontsize=16)
...
ax.set_title("Top 20 Anomalies (v9)")
ax.invert_yaxis()

# 4. Feature importance (PCA loadings)
ax = axes[1, 0]
feature_names = ["J-H", "H-K", "W1-W2", "W2-W3",
                 "variability", "compactness", "score_color"]
...
plt.tight_layout()
plot_file = os.path.join(PLOT_DIR, "classifier_results_v9.png")
...
import pickle
model_file = os.path.join(DATA_DIR, "classifier_model_v9.pkl")
...
# ==========
print("\n" + "="*80)
print("BlackHole Beacon — Anomaly Detection v9.0 (NO PM) COMPLETE")
print("="*80)

print(f"\n  Candidates analyzed: {len(candidates)}")
print(f"  Contamination (assumed): 5%")
print(f"  Anomaly score range: [{scores.min():.4f}, {scores.max():.4f}]")
...
print("\n" + "="*80)
print("DONE. Anomaly detection complete (NO PM features).")
print("="*80)
